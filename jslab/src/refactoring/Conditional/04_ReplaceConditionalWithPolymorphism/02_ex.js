...
function voyageProfitFactor(voyage, history) {
  ...
  if (voyage.zone === '중국' && hasChina(history)) {
    ...
    if (voayge.length > 12) result += 1;
    if (voayge.length > 18) result -= 1;
  } else {
    if (history.length > 8) result += 1;
    if (voayge.length > 14) result -= 1;
  }
  return result;
}
...
// 호출
const voayge = { zone: '서인도', length: 10 };
const history = [
  { zone: '동인도', profit: 5 },
  { zone: '서인도', profit: 15 },
  { zone: '중국', profit: -2 },
...
const myRating = rating(voayge, history);
